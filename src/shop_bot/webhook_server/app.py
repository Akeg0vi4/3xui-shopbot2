# filepath: /root/3xui-shopbot/src/shop_bot/webhook_server/app.py
import os
import logging
import asyncio
import json
...
def create_webhook_app(bot_controller_instance):
    ...
    def delete_user_route(user_id):
        """Полное удаление пользователя из БД вместе с ключами"""
        try:
            from shop_bot.data_manager.database import db_session, User, Key
            ...
            # Удаляем все ключи пользователя с серверов
            if hasattr(user, 'keys') and user.keys:
                for key in user.keys:
                    try:
                        asyncio.run(xui_api.delete_client_on_host(key.host_name, key.key_email))
                    except Exception as e:
                        logger.error(f"Ошибка удаления ключа {key.key_id} с сервера: {e}")
            
            # Удаляем пользователя из БД (каскадно удалятся и ключи)
            ...
            try:
                bot = _bot_controller.get_bot_instance()
                if bot:
                    text = "❌ Ваш аккаунт был полностью удален администратором. Все ключи отозваны."
                    loop = current_app.config.get('EVENT_LOOP')
                    if loop and loop.is_running():
                        asyncio.run_coroutine_threadsafe(bot.send_message(chat_id=user_id, text=text), loop)
            except Exception as e:
                logger.warning(f"Не удалось уведомить пользователя {user_id} об удалении: {e}")
            
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'ok': True, 'message': 'Пользователь полностью удален'})
            
            flash('Пользователь полностью удален', 'success')
            return redirect(url_for('users_page'))
    ...
    @login_required
    def send_message_to_user_route(user_id):
        """Отправка сообщения пользователю от имени бота поддержки"""
        try:
            ...
            # Отправляем сообщение через основного бота
            bot = _bot_controller.get_bot_instance()
            if not bot:
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return jsonify({'ok': False, 'error': 'Бот не запущен'})
    ...
    @flask_app.route('/users/extend/<int:user_id>', methods=['POST'])
    @login_required
    def extend_user_keys_route(user_id):
        """Продление всех ключей пользователя на указанное количество дней"""
        try:
            ...
            if not keys:
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return jsonify({'ok': False, 'error': 'У пользователя нет ключей'})
            ...
            for key in keys:
                try:
                    ...
                    # Если ключ истек или скоро истечет, продлеваем от текущей даты
                    from datetime import datetime
                    if isinstance(cur_expiry, str):
                        try:
                        ...
                        exp_dt = cur_expiry or datetime.now()
                    
                    # Если дата истекла, считаем от текущего момента
                    if exp_dt < datetime.now():
                        exp_dt = datetime.now()
                    
                    # Новая дата истечения
                    new_dt = exp_dt + timedelta(days=days)
                    new_ms = int(new_dt.timestamp() * 1000)
                    ...
                    if result and result.get('expiry_timestamp_ms'):
                        # Обновляем в БД
                        update_key_info(
                            key.get('key_id'), 
                            result.get('client_uuid') or key.get('xui_client_uuid'),
                            int(result.get('expiry_timestamp_ms'))
                        )
                        extended_count += 1
                    else:
                        errors.append(f"Ключ {key.get('key_id')}: не удалось обновить на сервере")
                        
                except Exception as e:
                    errors.append(f"Ключ {key.get('key_id')}: {str(e)}")
                    logger.error(f"Ошибка продления ключа {key.get('key_id')}: {e}")
            
            # Уведомление пользователя
            try:
                bot = _bot_controller.get_bot_instance()
                if bot and extended_count > 0:
                    text = f"✅ Ваши VPN-ключи продлены на {days} дней администратором.\nПродлено ключей: {extended_count}"
                    loop = current_app.config.get('EVENT_LOOP')
                    if loop and loop.is_running():
                        asyncio.run_coroutine_threadsafe(bot.send_message(chat_id=user_id, text=text), loop)
            except Exception as e:
                logger.warning(f"Не удалось уведомить пользователя {user_id} о продлении: {e}")
            
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({
                    'ok': True, 
                    'message': f'Продлено {extended_count} из {len(keys)} ключей',
            ...
            if errors:
                flash(f'Продлено {extended_count} из {len(keys)} ключей. Ошибки: {", ".join(errors[:3])}', 'warning')
            else:
                flash(f'Все {extended_count} ключей успешно продлены на {days} дней', 'success')
            ...
            logger.error(f"Ошибка массового продления ключей пользователя {user_id}: {e}", exc_info=True)
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'ok': False, 'error': str(e)})
            flash(f'Ошибка продления: {str(e)}', 'danger')
            return redirect(url_for('users_page'))

    @flask_app.route('/users/bulk-extend', methods=['POST'])
    @login_required
    def bulk_extend_keys_route():
        """Массовое продление всех ключей всех пользователей"""
        try:
            days = int(request.form.get('days', 30))
            if days <= 0:
                raise ValueError("Количество дней должно быть положительным")
            ...
            if not all_keys:
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return jsonify({'ok': False, 'error': 'Нет ключей для продления'})
                flash('Нет ключей для продления', 'warning')
                return redirect(url_for('users_page'))
            ...
            for key in all_keys:
                try:
                    # Пропускаем ключи без email или host_name
                    if not key.get('key_email') or not key.get('host_name'):
                        continue
                    ...
                    errors.append(f"Ключ {key.get('key_id')}: {str(e)}")
                    logger.error(f"Ошибка массового продления ключа {key.get('key_id')}: {e}")
            
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({
                    'ok': True,
                    'message': f'Продлено {extended_count} из {len(all_keys)} ключей',
                    'extended': extended_count,
                    'total': len(all_keys),
                    'errors': errors if errors else None
                })
            ...
            if errors:
                flash(f'Массовое продление: {extended_count} из {len(all_keys)} ключей. Ошибки: {", ".join(errors[:3])}', 'warning')
            else:
                flash(f'Массовое продление: все {extended_count} ключей успешно продлены на {days} дней', 'success')
            ...
            logger.error(f"Ошибка массового продления ключей: {e}", exc_info=True)
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'ok': False, 'error': str(e)})
            flash(f'Ошибка массового продления: {str(e)}', 'danger')
            return redirect(url_for('users_page'))
