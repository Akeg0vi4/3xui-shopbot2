    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            if not row:
                ...
                logger.info(f"Creating new user {telegram_id} with referrer {referrer_id}")
                cursor.execute(
                ...
                if referrer_id and (current_ref is None or str(current_ref).strip() == "") and int(referrer_id) != int(telegram_id):
                    logger.info(f"Updating existing user {telegram_id} with referrer {referrer_id}")
                    try:
...
def extend_user_keys(user_id: int, days_to_add: int) -> int:
    """Продлевает ВСЕ ключи пользователя на указанное количество дней.
    Возвращает количество обновлённых ключей."""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            
            # Получаем все ключи пользователя
            cursor.execute("SELECT key_id, expiry_date FROM vpn_keys WHERE user_id = ?", (user_id,))
            keys = cursor.fetchall()
            
            updated = 0
            for key_id, expiry_date_str in keys:
                if expiry_date_str:
                    # Парсим текущую дату
                    current_expiry = datetime.fromisoformat(expiry_date_str)
                    # Добавляем дни
                    ...
                    # Если даты нет, создаём новую от текущего момента
            ...
            conn.commit()
            logger.info(f"Extended {updated} keys for user {user_id} by {days_to_add} days")
            return updated
    except sqlite3.Error as e:
        logger.error(f"Failed to extend keys for user {user_id}: {e}")
        return 0

def extend_all_users_keys(days_to_add: int, min_days_left: int = 0) -> dict:
    """Продлевает ключи ВСЕМ пользователям у которых осталось меньше min_days_left дней.
    Возвращает статистику: сколько пользователей и ключей обновлено."""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            ...
            # Получаем все ключи
            cursor.execute("""
                SELECT key_id, user_id, expiry_date 
                FROM vpn_keys 
            ...
            for key_id, user_id, expiry_date_str in keys:
                try:
                    expiry = datetime.fromisoformat(expiry_date_str)
                    days_left = (expiry - now).days
                    
                    # Продлеваем только если осталось меньше указанного порога
                    if days_left <= min_days_left or min_days_left == 0:
                        new_expiry = expiry + timedelta(days=days_to_add)
                        cursor.execute(
                            "UPDATE vpn_keys SET expiry_date = ? WHERE key_id = ?",
                            (new_expiry.isoformat(), key_id)
                        )
                        stats["users_affected"].add(user_id)
                        stats["keys_updated"] += 1
                    else:
                        stats["users_skipped"].add(user_id)
            ...
            # Преобразуем множества в списки для возврата
            stats["users_affected"] = list(stats["users_affected"])
            stats["users_skipped"] = list(stats["users_skipped"])
            
            logger.info(f"Mass extension: {stats['keys_updated']} keys for {len(stats['users_affected'])} users")
            return stats
    except sqlite3.Error as e:
        logger.error(f"Failed to extend all keys: {e}")
        return {"error": str(e)}

def delete_user_completely(user_id: int) -> bool:
    """Полностью удаляет пользователя из БД вместе со всеми ключами"""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("BEGIN TRANSACTION")
            
            # Сначала удаляем ключи пользователя
            cursor.execute("DELETE FROM vpn_keys WHERE user_id = ?", (user_id,))
            ...
            conn.commit()
            logger.info(f"User {user_id} completely deleted. Keys deleted: {deleted_keys}")
            return deleted_user > 0
    except sqlite3.Error as e:
        logger.error(f"Failed to delete user {user_id}: {e}")
        return False

def start_support_chat(user_id: int) -> int | None:
    """Создаёт чат поддержки от имени админа с пользователем.
    Возвращает ticket_id или None при ошибке."""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            
            # Проверяем, есть ли уже открытый тикет от этого пользователя
            cursor.execute(
            ...
            if existing:
                ...
                # Создаём новый тикет
                cursor.execute(
                    "INSERT INTO support_tickets (user_id, status) VALUES (?, 'open')",
                    (user_id,)
                )
        ...
        logger.error(f"Failed to start support chat for user {user_id}: {e}")
        return None