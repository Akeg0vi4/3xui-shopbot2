...
from shop_bot.data_manager.database import (
    ...
    extend_user_keys,
    extend_all_users_keys,
    delete_user_completely,
    start_support_chat,
)
from shop_bot.data_manager import backup_manager
from shop_bot.bot.handlers import show_main_menu
from shop_bot.modules.xui_api import create_or_update_key_on_host, delete_client_on_host, get_key_details_from_host
...
def get_admin_router() -> Router:
    ...
    # --- Просмотр и редактирование ключа (с выводом конфига) ---
    ...
    async def admin_edit_key(callback: types.CallbackQuery):
        ...
        try:
            ...
            logger.error(f"Failed to get config for key {key_id}: {e}")
        ...
        try:
            ...
            logger.debug(f"edit_text failed in admin_edit_key for key #{key_id}: {e}")
            await callback.message.answer(
                text,
                reply_markup=keyboards.create_admin_key_actions_keyboard(key_id, int(key.get('user_id')) if key and key.get('user_id') else None)
            )

    # --- Копирование конфига ключа (админская кнопка) ---
    @admin_router.callback_query(F.data.startswith("admin_copy_config_"))
    async def admin_copy_config(callback: types.CallbackQuery):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        ...
        try:
            details = await get_key_details_from_host(key)
            if details and details.get('connection_string'):
                await callback.answer("✅ Конфиг скопирован!", show_alert=False)
                await callback.message.answer(
                    f"<code>{details['connection_string']}</code>",
                    parse_mode="HTML"
                )
            else:
                await callback.answer("Не удалось получить конфиг", show_alert=True)
        except Exception as e:
            logger.error(f"Error copying config for key {key_id}: {e}")
            await callback.answer("Ошибка при получении конфига", show_alert=True)
    ...
    async def admin_key_extend_process(message: types.Message, state: FSMContext):
        ...
        # Получаем конфиг для отображения
        connection_string = ""
        try:
            details = await get_key_details_from_host(new_key)
            if details and details.get('connection_string'):
                connection_string = details['connection_string']
        except Exception:
            pass
    ...
    async def admin_delete_user(callback: types.CallbackQuery):
        ...
        builder.button(text="✅ Да, удалить", callback_data=f"admin_confirm_delete_user_{user_id}")
        builder.button(text="❌ Отмена", callback_data=f"admin_view_user_{user_id}")
        builder.adjust(1)
        
        await callback.message.edit_text(
            f"⚠️ Ты действительно хочешь **полностью удалить** пользователя {user_id}?\n"
            f"Все его ключи тоже будут удалены!",
            reply_markup=builder.as_markup()
        )

    @admin_router.callback_query(F.data.startswith("admin_confirm_delete_user_"))
    async def admin_confirm_delete_user(callback: types.CallbackQuery):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        ...
        # Сначала получим информацию о пользователе для уведомления
        user = get_user(user_id)
        
        # Удаляем пользователя
        if delete_user_completely(user_id):
            await callback.message.edit_text(f"✅ Пользователь {user_id} и все его ключи удалены")
            
            # Уведомление пользователю, если он не забанен
            try:
            ...
            await callback.message.edit_text(f"❌ Ошибка при удалении пользователя {user_id}")
        
        # Возвращаемся к списку пользователей через 2 секунды
    ...
    # --- Написать сообщение пользователю (открыть чат поддержки) ---
    ...
    async def admin_message_user(callback: types.CallbackQuery, state: FSMContext):
        ...
        ticket_id = start_support_chat(user_id)
        
        if ticket_id:
            # Сохраняем информацию о тикете в состоянии
            ...
            await callback.message.edit_text(
                f"✍️ Напиши сообщение для пользователя {user_id}.\n"
                f"Оно будет отправлено ему от имени бота поддержки.\n\n"
                f"Тикет #{ticket_id} создан.",
                reply_markup=keyboards.create_admin_cancel_keyboard()
            )
            
            # Устанавливаем состояние ожидания сообщения
            from shop_bot.bot.handlers import SupportDialog
            await state.set_state(SupportDialog.waiting_for_message)
        else:
            await callback.message.edit_text("❌ Не удалось создать чат поддержки")

    # --- Меню продления ключей ---
    @admin_router.callback_query(F.data == "admin_extend_menu")
    async def admin_extend_menu(callback: types.CallbackQuery):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        
        builder = InlineKeyboardBuilder()
        builder.button(text="👤 Продлить конкретному пользователю", callback_data="admin_extend_single_form")
        builder.button(text="👥 Продлить всем пользователям", callback_data="admin_extend_all_form")
        builder.button(text="⬅️ Назад", callback_data="admin_menu")
        builder.adjust(1)
        
        await callback.message.edit_text(
            "🔧 <b>Продление ключей</b>\n\n"
            "Выбери тип продления:",
            reply_markup=builder.as_markup()
        )

    # --- Форма для продления конкретному пользователю ---
    class AdminExtendSingleUser(StatesGroup):
        waiting_for_user_id = State()
        waiting_for_days = State()

    @admin_router.callback_query(F.data == "admin_extend_single_form")
    async def admin_extend_single_form(callback: types.CallbackQuery, state: FSMContext):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        
        await callback.message.edit_text(
            "👤 <b>Продление конкретному пользователю</b>\n\n"
            "Введи Telegram ID пользователя:",
            reply_markup=keyboards.create_admin_cancel_keyboard()
        )
        await state.set_state(AdminExtendSingleUser.waiting_for_user_id)

    @admin_router.message(AdminExtendSingleUser.waiting_for_user_id)
    async def admin_extend_single_user_id(message: types.Message, state: FSMContext):
        if not is_admin(message.from_user.id):
            return
        ...
        await state.set_state(AdminExtendSingleUser.waiting_for_days)
        
        await message.answer(
            f"👤 Пользователь: {user_id}\n"
            f"Ключей: {len(get_keys_for_user(user_id))}\n\n"
            f"Введи количество дней для продления:",
            reply_markup=keyboards.create_admin_cancel_keyboard()
        )

    @admin_router.message(AdminExtendSingleUser.waiting_for_days)
    async def admin_extend_single_days(message: types.Message, state: FSMContext):
        if not is_admin(message.from_user.id):
            return
        ...
        # Продлеваем ключи
        updated = extend_user_keys(user_id, days)
        
        await state.clear()
        
        if updated > 0:
            await message.answer(f"✅ Пользователю {user_id} продлено {updated} ключей на {days} дней")
            
            # Уведомляем пользователя
            try:
                await message.bot.send_message(
                    user_id,
                    f"ℹ️ Администратор продлил ваши ключи на {days} дней."
                )
            except Exception:
                pass
        else:
            await message.answer(f"❌ У пользователя {user_id} нет ключей или произошла ошибка")
        
        await show_admin_menu(message)

    # --- Массовое продление ---
    class AdminExtendAll(StatesGroup):
        waiting_for_condition = State()
        waiting_for_days = State()

    @admin_router.callback_query(F.data == "admin_extend_all_form")
    async def admin_extend_all_form(callback: types.CallbackQuery, state: FSMContext):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        
        builder = InlineKeyboardBuilder()
        builder.button(text="✅ Всем (даже с большим сроком)", callback_data="admin_extend_all_0")
        builder.button(text="⏳ Только с остатком < 7 дней", callback_data="admin_extend_all_7")
        builder.button(text="⏳ Только с остатком < 30 дней", callback_data="admin_extend_all_30")
        builder.button(text="⬅️ Назад", callback_data="admin_extend_menu")
        builder.adjust(1)
        
        await callback.message.edit_text(
            "👥 <b>Массовое продление</b>\n\n"
            "Выбери условие для продления. Затем нужно будет ввести количество дней.",
            reply_markup=builder.as_markup()
        )
        await state.set_state(AdminExtendAll.waiting_for_condition)

    @admin_router.callback_query(AdminExtendAll.waiting_for_condition, F.data.startswith("admin_extend_all_"))
    async def admin_extend_all_condition(callback: types.CallbackQuery, state: FSMContext):
        if not is_admin(callback.from_user.id):
            await callback.answer("У вас нет прав.", show_alert=True)
            return
        ...
        await state.set_state(AdminExtendAll.waiting_for_days)
        
        await callback.message.edit_text(
            f"👥 <b>Массовое продление</b>\n\n"
            f"Минимальный остаток дней: {min_days if min_days > 0 else 'любой'}\n\n"
            f"Введи количество дней для продления:",
            reply_markup=keyboards.create_admin_cancel_keyboard()
        )

    @admin_router.message(AdminExtendAll.waiting_for_days)
    async def admin_extend_all_days(message: types.Message, state: FSMContext):
        if not is_admin(message.from_user.id):
            return
        ...
        # Продлеваем всех
        stats = extend_all_users_keys(days, min_days)
        ...
        if "error" in stats:
            ...
            await message.answer(
                ...
                f"📊 <b>Статистика:</b>\n"
                f"• Пользователей затронуто: {len(stats['users_affected'])}\n"
                f"• Ключей обновлено: {stats['keys_updated']}\n"
                f"• Пользователей пропущено: {len(stats['users_skipped'])}\n"
                f"• Ключей пропущено: {stats['keys_skipped']}"
            )
